...
it('can change name', async () => {
    let instance = await StarNotary.deployed(); // Making sure the Smart Contract is deployed and getting the instance.
    await instance.claimStar({from: owner});
    await instance.changeName("Agam", {from: owner});
    let starName = await instance.starName.call(); // Getting the owner address
    assert.equal(starName, "Agam"); // Verifying if the owner address match with owner of the address
});